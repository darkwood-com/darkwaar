...
export class Start extends Command {
  ...
  /**
   * Stoppes darkwaar in a graceful way.
   */
  ...
  async run() {
    // Make sure that darkwaar shuts down gracefully if possible
    ...
    (async () => {
      try {
        ...
        const url = `http://localhost:${config.get('port')}`;
        this.log(`\nDarkwaar client v${require('../../package.json').version} is ready on:\n${url}`);

        // Allow to open darkwaar editor by pressing "o"
